...
// Apply IP banning middleware globally
app.use(banAbusiveIPs);

// Authentication routes with strict rate limiting (before auth middleware)
...
app.use((req, res, next) => {
  ...
  if (req.path.startsWith('/api/auth') || req.path.startsWith('/login') || req.path === '/favicon.ico') {
    return next();
  }
...
app.post('/api/branding', (req, res) => {
  try {
    ...
    const sanitizedBody = {
      ...req.body,
      appName: req.body.appName || 'Tank Monitoring System',
      appSlogan: req.body.appSlogan || 'Real-time tank level monitoring dashboard',
    };
    appBranding = { ...appBranding, ...sanitizedBody };
...
app.get('/api/flexible/browse', async (req, res) => {
...
app.post('/api/flexible/sources', async (req, res) => {
  try {
    ...
    if (sanitizedBody.path) {
      // Remove any potentially dangerous characters
      sanitizedBody.path = sanitizedBody.path.replace(/[<>:"|?*]/g, '');
    }

    const source = fileMonitor.addSource(sanitizedBody);
...
// Admin endpoint to monitor server status (requires admin role)
  ...
  fileMonitor.on('data', (event) => {
    ...
    connectedClients.forEach(client => {
      try {
        ...
        console.error('Error broadcasting file monitor data:', error);
        deadClients.push(client);
      }
    });
  ...
  fileMonitor.on('error', (event) => {
    ...
    connectedClients.forEach(client => {
      try {
        ...
        console.error('Error broadcasting error message:', error);
        deadClients.push(client);
      }
    });
...
function setupWebSocket() {
  ...
  const healthCheckInterval = setInterval(() => {
    ...
    connectedClients.forEach((ws) => {
      ...
      if (now - ws.lastActivity > CONNECTION_TIMEOUT) {
        console.log('Removing stale connection (timeout)');
      ...
      if (ws.isAlive === false) {
        console.log('Removing dead connection (failed ping)');
        clientsToRemove.push(ws);
        return;
      }
    ...
    if (clientsToRemove.length > 0) {
      console.log(`Cleaned up ${clientsToRemove.length} dead connections. Active connections: ${connectedClients.size}`);
    }
  ...
  console.log(`WebSocket server running on port ${WS_PORT} with health checks every ${HEALTH_CHECK_INTERVAL / 1000}s`);
}
...
async function init() {
  ...
  // Set up static path and middleware with rate limiting
