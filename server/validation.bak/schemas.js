...
const commonFields = {
  ...
  filePath: joi.custom().safePath().max(500),
  url: joi.custom().secureUrl().max(2000),
  port: joi.custom().port(),
...
export const csvFileConfigSchema = joi.object({
  enabled: joi.boolean().default(false),
  filePath: commonFields.filePath.when('enabled', {
    is: true,
    then: joi.required(),
  }),
...
export const brandingSchema = joi.object({
  ...
  logoUrl: commonFields.url.allow('').optional(),
  favicon: commonFields.url.allow('').optional(),
});
