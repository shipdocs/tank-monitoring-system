...
const commonFields = {
  ...
  filePath: joi.safeCustom().safePath().max(500),
  url: joi.safeCustom().secureUrl().max(2000),
  port: joi.safeCustom().port(),
...
// CSV file configuration schema
export const csvFileConfigSchema = joi.object({
  enabled: joi.boolean().default(false),
  filePath: joi.safeCustom().safePath().max(500).when('enabled', {
    is: true,
    then: joi.required(),
  }),
...
export const mainConfigSchema = joi.object({
  selectedPort: serialConfigSchema.extract('selectedPort'),
  baudRate: serialConfigSchema.extract('baudRate'),
  dataBits: serialConfigSchema.extract('dataBits'),
  stopBits: serialConfigSchema.extract('stopBits'),
  parity: serialConfigSchema.extract('parity'),
  autoConnect: serialConfigSchema.extract('autoConnect'),
  tankCount: serialConfigSchema.extract('tankCount'),
  dataFormat: serialConfigSchema.extract('dataFormat'),
  customParser: serialConfigSchema.extract('customParser'),
  csvFile: csvFileConfigSchema,
...
export const brandingSchema = joi.object({
  ...
  logoUrl: joi.safeCustom().secureUrl().max(2000).allow('').optional(),
  favicon: joi.safeCustom().secureUrl().max(2000).allow('').optional(),
});

// Query parameter schemas
export const browseQuerySchema = joi.object({
  path: joi.safeCustom().safePath().max(500).default(process.cwd()),
});
